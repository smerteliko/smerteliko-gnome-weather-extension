...
class SmertelikoGnomeWeatherExtensionMenuButton extends PanelMenu.Button {
  static {
  ...
  _addWeatherToBox(topBox)
  {
    this._weatherIcon = new St.Icon({
      ...
      style_class: "system-status-icon smerteliko-gnome-weather-exstension-icon",
    });
    this._weatherInfo = new St.Label({
      style_class: "smerteliko-gnome-weather-exstension-label",
      text: "...",
  ...
  _init( metadata, settings) {
    ...
    this.loadConfig().then(() =>
    {
        ...
        this._timeoutFirstBoot = GLib.timeout_add_seconds(
          ...
          () => {
            try
            {
              ...
              this.initSmertelikoGnomeWeatherExtensionUI();
              _firstBoot = 0;
              this._timeoutFirstBoot = null;
            }
            catch(e)
            {
              console.log("Smerteliko-gnome-weather-exstension: Error in first boot timeout.");
              console.error(e);
            }
      ...
      else
      {
        ...
        {
          console.log("Smerteliko-gnome-weather-exstension: Error in immediate first boot.");
          console.error(e);
        }
      }
    }, (e) =>
    {
      console.error(`Smerteliko-gnome-weather-exstension: Error '${e}' in loadConfig.`);
      console.error(e);
      Main.notify("Smerteliko-gnome-weather-exstension", _("Failed to initialize."));
      let now = new Date();
      this.settings.set_string("last-init-error", `(${toYYYYMMDD(now)}) ${e}`);
    });
  }

  initSmertelikoGnomeWeatherExtensionUI() {
    this.owmCityId = 0;
    ...
    this._buttonMenu = new PopupMenu.PopupBaseMenuItem({
      reactive: false,
      style_class: "smerteliko-gnome-weather-exstension-menu-button-container",
    });
  ...
  async firstRunSetDefaults()
  {
    if(this.isFirstRun(true))
    {
      ...
      if(this.settings.get_enum("my-loc-prov") === MyLocProv.GEOCLUE)
      {
        ...
        {
          console.warn(`Smerteliko-gnome-weather-exstension: Geoclue failed ('${e}'); changing provider to ipinfo.io.`);
  ...
  async settingsHandler()
  {
    ...
    {
      console.error(`Smerteliko-gnome-weather-exstension: Error '${e}' in firstRunSetDefaults.`);
      throw e;
    }
  ...
  async settingsChangedHandlerWrapper()
  {
    ...
    {
        console.log("Smerteliko-gnome-weather-exstension Error in settings listener.");
        console.error(e);
    }
  }
  ...
  loadConfigInterface()
  {
    this._settingsInterfaceC = this.settings.connect("changed", async () => {
      ...
      {
        console.error(`Smerteliko-gnome-weather-exstension: Error in settings changed listener '${e}'.\n\t${e.trace}`);
      }
    });
  }
  ...
  get _actual_city()
  {
    ...
    {
      console.warn("Smerteliko-gnome-weather-exstension: Got actual city too high.");
  ...
  set _actual_city(i)
  {
    if(i > this._cities.length - 1)
    {
      console.warn("Smerteliko-gnome-weather-exstension: Set actual city too high.");
  ...
  createButton(iconName, accessibleName)
  {
    ...
    button = new St.Button({
      ...
      style_class: this.cssConcatClass("message-list-clear-button button smerteliko-gnome-weather-exstension-button-action", a11yClasses),
    });
  ...
  rebuildButtonMenu() {
    ...
    this._buttonBox1 = new St.BoxLayout({
      style_class: "smerteliko-gnome-weather-exstension-button-box",
    });
  ...
  rebuildCurrentWeatherUi()
  {
    ...
    this._currentWeatherIcon = new St.Icon({
      icon_size: 96,
      icon_name: "view-refresh-symbolic",
      style_class: "system-menu-action smerteliko-gnome-weather-exstension-current-icon",
    });

    this._sunriseIcon = new St.Icon({
      icon_size: 15,
      style_class: "smerteliko-gnome-weather-exstension-sunrise-icon",
    });
    this._sunsetIcon = new St.Icon({
      icon_size: 15,
      style_class: "smerteliko-gnome-weather-exstension-sunset-icon",
    });
    ...
    this._buildIcon = new St.Icon({
      icon_size: 15,
      icon_name: "view-refresh-symbolic",
      style_class: "smerteliko-gnome-weather-exstension-build-icon",
    });

    // The summary of the current weather
    this._currentWeatherSummary = new St.Label({
      text: _("Loading ..."),
      style_class: this.cssConcatClass("smerteliko-gnome-weather-exstension-current-summary", a11yClasses),
    });
    ...
    let bb = new St.BoxLayout({
      ...
      y_expand: true,
      y_align: Clutter.ActorAlign.CENTER,
      style_class: "system-menu-action smerteliko-gnome-weather-exstension-current-summarybox",
    });
    ...
    let ab = new St.BoxLayout({
      x_expand: true,
      style_class: "smerteliko-gnome-weather-exstension-current-infobox",
    });
    ...
    let rb = new St.BoxLayout({
      x_expand: true,
      style_class: "smerteliko-gnome-weather-exstension-current-databox",
    });
    let rb_captions = new St.BoxLayout({
      x_expand: true,
      vertical: true,
      style_class:
        "popup-menu-item popup-status-menu-item smerteliko-gnome-weather-exstension-current-databox-captions",
    });
    let rb_values = new St.BoxLayout({
      x_expand: true,
      vertical: true,
      style_class: "system-menu-action smerteliko-gnome-weather-exstension-current-databox-values",
    });
    ...
    const labelCss = this.cssConcatClass("smerteliko-gnome-weather-exstension-current-databox-captions", a11yClasses);
    const valueCss = this.cssConcatClass("smerteliko-gnome-weather-exstension-current-databox-values", a11yClasses);
    for(let i = 0; i < 5; i++)
    {
      let l = new St.Label({
        text: sideStats[i],
    ...
    let box = new St.BoxLayout({
      x_expand: true,
      style_class: "smerteliko-gnome-weather-exstension-current-iconbox",
    });
    ...
    this._todaysBox = new St.BoxLayout({
      ...
      style_class: "smerteliko-gnome-weather-exstension-today-box",
    });
    ...
    for (let i = 0; i < 4; i++) {
      ...
      todaysForecast.Time = new St.Label({
        style_class: this.cssConcatClass("smerteliko-gnome-weather-exstension-forecast-time", a11yClasses),
      });
      todaysForecast.Icon = new St.Icon({
        icon_size: 24,
        icon_name: "view-refresh-symbolic",
        style_class: "smerteliko-gnome-weather-exstension-forecast-icon",
      });
      todaysForecast.Temperature = new St.Label({
        style_class: this.cssConcatClass("smerteliko-gnome-weather-exstension-forecast-temperature", a11yClasses),
      });
      todaysForecast.Summary = new St.Label({
        style_class: this.cssConcatClass("smerteliko-gnome-weather-exstension-forecast-summary", a11yClasses),
      });
      ...
      let fb = new St.BoxLayout({
        vertical: true,
        x_expand: true,
        style_class: "smerteliko-gnome-weather-exstension-today-databox",
      });
      let fib = new St.BoxLayout({
        x_expand: true,
        x_align: Clutter.ActorAlign.CENTER,
        style_class: "smerteliko-gnome-weather-exstension-forecast-iconbox",
      });
  ...
  rebuildFutureWeatherUi(cnt) {
    ...
    this._forecastExpanderBox = new St.BoxLayout({
      x_expand: true,
      opacity: 150,
      style_class: this.cssConcatClass("smerteliko-gnome-weather-exstension-forecast-expander", a11yClasses),
    });
    ...
    this._daysBox = new St.BoxLayout({
      vertical: true,
      y_expand: true,
      style_class: "smerteliko-gnome-weather-exstension-forecast-box",
    });
    ...
    this._forecastScrollBox = new St.ScrollView({
      x_expand: true,
      style_class: "smerteliko-gnome-weather-exstension-forecasts",
    });
    ...
    for (let i = 0; i < cnt; i++) {
      ...
      forecastWeather.Day = new St.Label({
        style_class: "smerteliko-gnome-weather-exstension-forecast-day",
      });
      ...
      for (let j = 0; j < 8; j++) {
        ...
        forecastWeather[j].Time = new St.Label({
          style_class: "smerteliko-gnome-weather-exstension-forecast-time",
        });
        ...
        forecastWeather[j].Temperature = new St.Label({
          style_class: "smerteliko-gnome-weather-exstension-forecast-temperature",
        });
        forecastWeather[j].Summary = new St.Label({
          style_class: "smerteliko-gnome-weather-exstension-forecast-summary",
        });
        ...
        let by = new St.BoxLayout({
          vertical: true,
          x_expand: true,
          style_class: "smerteliko-gnome-weather-exstension-forecast-databox",
        });
...
export default class SmertelikoGnomeWeatherExtension extends Extension {
  enable() {
    ...
    Main.panel.addToStatusArea("SmertelikoGnomeWeatherExtensionMenu", this.SmertelikoGnomeWeatherExtensionMenu);
  }
