...
async function httpGetLoc(locProv)
{
  ...
  switch(locProv)
  {
    case MyLocProv.INFOIPIO:
      ...
      break;
    default:
      console.error(`Smerteliko-gnome-weather-exstension: HTTP Get Loc called when it shouldn't have been.`);
    ...
    sess.send_and_read_async(
      ...
      (s, m) =>
      {
        ...
        if(!response || !(response = response.get_data()))
        {
          ...
          fetchingLocation = false;
          locationTime = new Date();
          reject("Smerteliko-gnome-weather-exstension: Invalid response");
          return;
        }
        ...
        if(!str)
        {
          ...
          fetchingLocation = false;
          locationTime = new Date();
          reject("Smerteliko-gnome-weather-exstension: No data in JSON My Location HTTP response.");
          return;
        }
...
export async function geoclueGetLoc(useNominatim = true)
{
    ...
    Geoclue.Simple.new(
      ...
      (_s, result) => {
        ...
        catch(e)
        {
          fetchingLocation = false;
          locationTime = new Date();
          reject(`Smerteliko-gnome-weather-exstension: ${e}`);
          return;
        }
  ...
  return await new Promise((resolve, reject) => {
    ...
    sess.user_agent = "smerteliko-gnome-weather-extension@smerteliko.com";
    ...
    sess.send_and_read_async(
      ...
      (s, m) =>
      {
        ...
        if(!response || !(response = response.get_data()))
        {
          ...
          fetchingLocation = false;
          locationTime = new Date();
          reject("Smerteliko-gnome-weather-exstension: Invalid response from Nominatim.");
          return;
        }
        ...
        if(!str)
        {
          ...
          fetchingLocation = false;
          locationTime = new Date();
          reject("Smerteliko-gnome-weather-exstension: No data in JSON Nominatim HTTP response.");
          return;
        }
        let obj = JSON.parse(str);
...
export async function getLocationInfo(settings, forceRefresh = false)
{
  ...
  {
    console.error("Smerteliko-gnome-weather-exstension: getLocationInfo did not receive a settings argument. Pass 'null' to get settings automatically.");
  ...
  if(fetchingLocation)
  {
    console.warn("Smerteliko-gnome-weather-exstension: Location requested while fetching it; returning cached location.");
  }
  else
  {
