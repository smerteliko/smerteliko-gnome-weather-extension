...
export class Loc
{
  ...
  constructor(nameType, name, placeType, place)
  {
    ...
    {
      error = true;
      console.error(`Smerteliko-gnome-weather-exstension: NameType (${nameType}) not a number.`);
    }
    ...
    {
      error = true;
      console.error(`Smerteliko-gnome-weather-exstension: Name (${name}) not a string.`);
    }
    this.#name = name;
    ...
    {
      error = true;
      console.error(`Smerteliko-gnome-weather-exstension: PlaceType (${placeType}) not a number.`);
    }
    ...
    {
      error = true;
      console.error(`Smerteliko-gnome-weather-exstension: Place (${place}) not a string.`);
    }
    this.#place = place;

    if(error) console.trace("Smerteliko-gnome-weather-exstension: Loc ctor backtrace");
  }
  ...
  getName(gettext)
  {
    ...
    {
      console.error("Smerteliko-gnome-weather-exstension: Loc#getName did not receive a gettext argument. Pass 'null' for no gettext.");
      console.trace("Smerteliko-gnome-weather-exstension backtrace");
    }
    ...
    switch(this.#nameType)
    {
      ...
      default:
        console.warn(`Smerteliko-gnome-weather-exstension: Invalid name type (${this.#nameType}).`);
        return null;
    }
  }
  ...
  async getCoords(settings)
  {
    ...
    switch(this.#placeType)
    {
      ...
      default:
        console.warn(`Smerteliko-gnome-weather-exstension: Invalid place type (${this.#placeType}).`);
        return null;
    }
  }
  ...
  getPlaceDisplay(gettext)
  {
    if(gettext === undefined)
    {
      console.error("Smerteliko-gnome-weather-exstension: Loc#getPlaceDisplay did not receive a gettext argument. Pass 'null' for no gettext.");
    }
...
function fromLocsGVariant(val)
{
  ...
  for(let i = 0; i < locCount; i++)
  {
    ...
    if(tupleCount !== 4)
    {
      console.error(`Smerteliko-gnome-weather-exstension: 'locs' tuple of count ${tupleCount}, not 4.`);
      return [ ];
    }
